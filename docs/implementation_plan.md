...
- [ ] 트위터 클론 필수 테이블 추가
  ...
  - **확인 방법**:
    ...
    - Prisma Studio에서 테이블이 생성되었는지 확인 (`npx prisma studio`)
...
- [ ] 댓글 작성
  ...
  - 댓글 즉시 표시
  - **확인 방법**:
    - 트윗 상세 페이지에 댓글 작성 폼이 표시되는지 확인
    - 댓글 작성 후 댓글이 즉시 목록에 추가되는지 확인
    - 댓글 작성 후 입력 필드가 초기화되는지 확인
    - 댓글이 데이터베이스에 저장되었는지 확인
    - 로그인하지 않은 사용자는 댓글 작성이 불가능한지 확인
...
- [ ] 댓글 목록
  ...
  - 댓글 카운트 표시
  - **확인 방법**:
    - 트윗 상세 페이지에 댓글 목록이 표시되는지 확인
    - 댓글이 시간순(최신순 또는 오래된순)으로 정렬되어 표시되는지 확인
    - 트윗 카드에 댓글 개수가 올바르게 표시되는지 확인
    - 댓글이 없는 트윗의 경우 적절한 메시지가 표시되는지 확인
    - 댓글 작성 시 댓글 카운트가 업데이트되는지 확인
...
- [ ] 대댓글 기능
  ...
  - 대댓글 작성 action
  - **확인 방법**:
    - 댓글에 답글 달기 버튼이 표시되는지 확인
    - 답글 달기 클릭 시 대댓글 작성 폼이 표시되는지 확인
    - 대댓글이 원본 댓글 아래 들여쓰기되어 표시되는지 확인
    - 대댓글 작성 후 트리 구조가 올바르게 표시되는지 확인
    - 대댓글이 데이터베이스에 저장되고 부모 댓글과 연결되었는지 확인
...
- [ ] 댓글 수정/삭제
  ...
  - 댓글 삭제 action
  - **확인 방법**:
    - 본인이 작성한 댓글에만 수정/삭제 버튼이 표시되는지 확인
    - 다른 사용자의 댓글에는 수정/삭제 버튼이 표시되지 않는지 확인
    - 댓글 수정 후 변경사항이 반영되어 표시되는지 확인
    - 댓글 삭제 후 댓글이 목록에서 제거되는지 확인
    - 댓글 삭제 시 댓글 카운트가 업데이트되는지 확인
    - 대댓글이 있는 댓글 삭제 시 처리 방식 확인 (삭제 또는 숨김 처리)
...
- [ ] 이미지 업로드
  ...
  - 업로드 진행 상태 표시
  - **확인 방법**:
    - 트윗 작성 폼에서 이미지 첨부 버튼 클릭 시 파일 선택 다이얼로그가 열리는지 확인
    - 선택한 이미지가 미리보기로 표시되는지 확인
    - 여러 이미지 선택이 가능한지 확인
    - 이미지 업로드 시 진행 상태(프로그레스 바 등)가 표시되는지 확인
    - 업로드 완료 후 이미지 URL이 트윗 데이터에 포함되는지 확인
    - Cloudinary에 이미지가 올바르게 업로드되었는지 확인
...
- [ ] 동영상 업로드
  ...
  - 업로드 진행 상태 표시
  - **확인 방법**:
    - 트윗 작성 폼에서 동영상 첨부 버튼 클릭 시 파일 선택 다이얼로그가 열리는지 확인
    - 선택한 동영상의 썸네일이나 미리보기가 표시되는지 확인
    - 동영상 업로드 시 진행 상태가 표시되는지 확인
    - 업로드 완료 후 동영상 URL이 트윗 데이터에 포함되는지 확인
    - Cloudinary에 동영상이 올바르게 업로드되었는지 확인
    - 파일 크기 제한이 올바르게 동작하는지 확인
...
- [ ] React Router v7 SPA 모드 설정
  ...
  - 빌드 설정 최적화
  - **확인 방법**:
    - `react-router.config.ts`에서 SPA 모드가 올바르게 설정되었는지 확인
    - 프로덕션 빌드가 성공적으로 완료되는지 확인 (`npm run build`)
    - 빌드된 파일이 `build/` 디렉토리에 생성되었는지 확인
    - 모바일 앱에서 라우팅이 올바르게 동작하는지 확인 (테스트 필요)
