...
### AI 하이브리드 검색 (RAG) 시스템 상세

STAYnC 블로그는 단순히 텍스트를 매칭하는 검색을 넘어, 문맥과 의미를 이해하는 하이브리드 검색 시스템을 구축했습니다.

#### 1. 의미 기반 벡터 검색 (Semantic Search)
- **임베딩**: Google Gemini `text-embedding-004` 모델을 사용하여 각 트윗의 내용과 태그 정보를 768차원의 고밀도 벡터로 변환합니다.
- **저장**: 생성된 벡터는 `TweetEmbedding` 테이블의 `vector` 컬럼에 `BLOB` 형태로 저장됩니다.
- **검색**: Turso(libSQL)의 벡터 확장 기능을 활용하여 `vector_distance_cos` 함수로 검색어와의 코사인 유사도를 계산합니다.
  - **임계값(Threshold)**: 0.4 미만 (0에 가까울수록 유사함)

#### 2. 키워드 및 태그 우선 검색
- 사용자가 입력한 검색어가 `TravelTag` 테이블의 `name`과 정확히 일치할 경우, 해당 태그가 포함된 트윗을 우선적으로 필터링하여 검색 결과의 정확도를 높입니다.

#### 3. 하이브리드 결과 병합
- 키워드 기반 결과와 AI 기반 의미 검색 결과를 결합한 후, 중복을 제거합니다.
- 최종 결과물은 사용자의 기대를 반영하여 **최신 작성 순(createdAt DESC)**으로 정렬하여 제공합니다.
