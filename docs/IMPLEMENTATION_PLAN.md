...
- [ ] 여행 일지 전용 UI (Log Mode Overlay)
  - 상시 노출되는 'AI 여행 일지' 버튼 추가
  - 촬영-녹음-생성으로 이어지는 전용 오버레이/페이지 UI 구현
  - **확인 방법**: 버튼 클릭 시 전용 UI가 매끄럽게 활성화되는지 확인

- [ ] 음성 녹음 및 STT 통합
  - Capacitor 오디오 레코더 연동
  - 녹음된 음성을 OpenAI Whisper 또는 Google STT로 텍스트화
  - **확인 방법**: 한국어 음성이 텍스트로 오차 없이 변환되는지 확인

- [ ] 사진(Vision) & 위치(POI) 데이터 통합
  - Gemini 1.5 비전 분석 연동 (이미지 내 장소/분위기/음식 추출)
  - **사진의 GPS 기반 구체적인 장소명(카페, 식당 등 POI) 검색 연동**
  - 유저 선택형 문체 엔진 (감성/정보/위트 등)
  - **확인 방법**: 사진 촬영 장소의 실제 상호명(예: '카페 노티드')이 문장에 자연스럽게 반영되는지 확인

- [ ] AI 문장 합성 및 발행 프로세스
  - STT 결과 + 비전 데이터 + **위치 정보** + 문체를 결합한 최종 트윗 초안 생성
  - 유저 컨펌 후 즉시 발행 기능
  - **확인 방법**: 사용자의 의도, 사진의 분위기, 그리고 정확한 장소명이 조화된 완성도 높은 문장이 생성되는지 확인
