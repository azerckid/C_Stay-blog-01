...
- [x] Phase 2: 인증 시스템 구현
- [x] Phase 3: 기본 레이아웃 및 UI 구성
- [x] Phase 4: 트윗 기능 (CRUD)
- [x] Phase 5: 소셜 기능 (좋아요, 리트윗)
- [x] **팔로우/언팔로우 UI & 낙관적 업데이트**
    - [x] 팔로우 버튼 컴포넌트 (`FollowButton`) 구현
    - [x] 팔로우 API 연동 (`/api/follows`)
    - [x] 언팔로우 시 확인 모달 (선택 사항) -> 토글 방식으로 구현하여 즉시 반영
    - [x] 비공개 계정 팔로우 요청 (Pending) 처리 및 알림 (수락/거절 상태 지속성 확보)
- [x] **공개/비공개 계정 및 트윗 공개 범위**
    - [x] 사용자 설정(`isPrivate`)에 따른 팔로우 승인 로직
    - [x] 트윗 작성 시 공개 범위 설정 (전체/팔로워/비공개)
    - [x] 공개 범위에 따른 트윗 노출 필터링 (홈 피드, 유저 프로필)
    - [x] 트윗 카드에 공개 범위 아이콘 표시 (작성 날짜 옆)
    - [x] 트윗 수정 시 공개 범위 변경 기능 추가
- [x] Phase 8: 미디어 업로드 (이미지/동영상)
- [x] Phase 8.5: 여행 블로그 특화 기능 (위치 정보, 지도 통합)
- [x] Phase 9: 프로필 및 사용자 기능 ✅
- [x] Phase 10: 검색 기능
...
- [x] Phase 7: 댓글 및 대댓글
    ...
    - [x] 댓글 알림 연동 (답글 클릭 시 부모 트윗 이동 및 하이라이팅)
- [x] 트윗 상세 페이지
  - [x] 트윗 ID로 단일 트윗 조회
...
- [x] 위치 정보 입력 기능
  - [x] 트윗 작성 시 위치 검색 UI (Google Places Autocomplete)
  - [x] 장소 자동완성 (지도 API 활용)
  - [x] 위도/경도 자동 저장
  - [x] 국가/도시 자동 추출 및 저장
  - **확인 방법**:
...
- [x] 여행 태그 시스템
  - [x] 여행 태그 생성 및 관리 (관리자 기능 또는 자동 생성)
  - [x] 트윗 작성 시 태그 추가 UI
  - [x] 태그 자동완성
  - [x] 트윗에 여러 태그 연결 (다대다 관계)
  - **확인 방법**:
...
- [x] 지도 통합
  - [x] 지도 라이브러리 설치 및 설정 (Google Maps Integration)
  - [x] 트윗 상세 페이지에 지도 표시 (위치가 있는 경우)
  - [x] 지도에 마커 표시 (위치명 포함)
  - [x] 지도 클릭으로 위치 선택 기능 (Location Picker로 대체 구현)
  - **확인 방법**:
...
- [x] 위치 기반 검색 및 필터링
  - [x] 국가/도시별 트윗 필터링 및 날짜 범위 필터링
  - [x] 위치 기반 검색 API 구현 (search.tsx loader 연동)
  - [x] 검색 UI 내 필터 Popover 및 LocationPicker 연동
  - **확인 방법**:
    - 검색 페이지에서 국가/도시 필터가 동작하는지 확인
    - 날짜 범위 필터 적용 시 해당 기간의 트윗만 표시되는지 확인
    - 필터 적용 시 배지가 표시되고 개별 삭제가 가능한지 확인
    - 필터와 키워드 검색이 공존하는지 확인 (And 조건)

- [x] 여행 일정 연계 (Travel Plan Integration) ✅
  - [x] `TravelPlan` 테이블 정의 및 `Tweet` 연결 (`travelPlanId` 연동)
  - [x] `/api/travel-plans` 엔드포인트 구현 (사용자별 일정 조회 및 생성)
  - [x] 트윗 작성 시 여행 일정 선택 UI (`TweetCompose` 내 Airplane 아이콘)
  - [x] 트윗 카드 내 여행 일정 뱃지 표시 (`Airplane01Icon` 및 일정 제목)
  - [x] 여행 일정 목록 페이지 (`/travel-plans`) 구현 및 네비게이션 연동
  - [x] 여행 일정 상세 페이지 (`/travel-plans/:id`) 및 그룹화된 트윗 표시
  - [x] 여행 일정 생성 다이얼로그 및 API 연동 (목록 페이지 내 "+" 버튼)
  - [x] 여행 일정 상세 내 세부 일정(Itinerary Items) 관리 UI 구현 (Tabs 방식)
  - [x] 트윗 수정 시 여행 일정 변경 및 해제 기능 추가
  - [x] 여행 일정 상세 페이지 내 전체 경로 지도 표시 (`TravelMap` 연동)
  - **확인 방법**:
    - `/travel-plans` 페이지에서 목록 및 아이템 개수가 올바르게 표시되는지 확인
    - 트윗 작성/수정 시 여행 계획을 선택/변경/해제할 수 있는지 확인
    - 여행 일정 상세 페이지에서 세부 일정(Add/Edit/Delete)이 정상 동작하는지 확인
    - Tabs(일정, 게시물) 전환이 정상적으로 이루어지는지 확인

- [x] 여행 날짜 기능
  - [x] 트윗 작성 시 여행 날짜 선택 UI
  - [x] 날짜별 트윗 정렬 및 필터링 - 날짜 필터링 기능 (검색 필터 연동 완료)
  - **확인 방법**:
...
- [x] 사용자 트윗 필터 (작성 글, 답글, 좋아요 목록)
  - [x] 트윗 탭: 사용자가 작성한 트윗 목록
  - [x] 답글 탭: 사용자가 작성한 답글 목록
  - [x] 좋아요 탭: 사용자가 좋아요한 트윗 목록
  - [x] 각 탭별 트윗 목록 표시
  - **확인 방법**:
    - 프로필 페이지에 게시물/답글/좋아요 탭이 표시되는지 확인
...
- [x] 트윗 검색
  ...
  - [x] 해시태그 검색
  - [x] 검색 결과 표시
  - [x] 검색 성능 최적화 (무한 스크롤 구현 완료)
  - [x] AI 임베딩 기반 검색 연동 (Gemini)
  - **확인 방법**:
    - 검색어로 트윗 내용을 검색할 수 있는지 확인
    - 해시태그(#태그)로 검색할 수 있는지 확인
    - 유사한 의미의 검색어(예: '열차' -> '기차')로도 검색 결과가 나오는지 확인 (AI 검색)
    - 검색 결과가 검색어 일치도와 유사도에 따라 정렬되어 표시되는지 확인
    - 스크롤 시 추가 데이터가 로드되는지 확인
...
### 목표
- 의미 기반 검색을 위한 벡터 검색 시스템 구현
- 키워드 매칭과 벡터 유사도 매칭을 결합한 하이브리드 검색 제공
- 언어와 문맥을 이해하는 스마트한 검색 경험 구축

### 구현 상세
- **벡터 데이터베이스**: Turso libSQL 벡터 확장 활용
- **임베딩 모델**: Google Gemini API (`text-embedding-004`)
- **검색 방식**: 
  - 키워드/태그 일치 검색 (Exact/Partial Match)
  - 코사인 유사도 기반 벡터 검색 (Distance Threshold: 0.4)
  - 두 결과의 중복 제거 및 시간순 최신화 정렬

### 작업 항목

- [x] 벡터 데이터베이스 설정
  - [x] Prisma 스키마 내 `TweetEmbedding` 모델 추가
  - [x] Turso 리모트 DB에 `vector` 전용 인덱스 설정
  - [x] `libsql` 어댑터를 통한 벡터 전용 쿼리 연동
  - [**확인 완료**]: 리모트 DB와 로컬 DB 환경 모두에서 벡터 검색 쿼리가 정상 작동함

- [x] 임베딩 생성 파이프라인
  - [x] Gemini API 연동 (`gemini.server.ts`)
  - [x] 트윗 작성/수정 시 실시간 임베딩 생성 및 저장 (`api.tweets.ts`)
  - [x] 기존 트윗 전수 조사를 위한 배치 인덱싱 스크립트 실행 완료
  - [**확인 완료**]: 새로운 트윗이 작성될 때마다 메타데이터와 함께 임베딩이 자동 생성됨

- [x] 벡터 저장 및 인덱싱
  - [x] `TweetEmbedding` 테이블 내 768차원 벡터 저장
  - [x] 트윗 ID와의 1:1 관계 정립 및 인덱싱
  - [**확인 완료**]: `vector_distance_cos` 함수를 이용한 유사도 검색이 유효한 시간 내 수행됨

- [x] 의미 기반 검색 API 구현
  - [x] 검색어의 실시간 Gemini 임베딩 변환
  - [x] 키워드 매칭과 벡터 검색 결과의 지능적 결합
  - [x] 검색어와 태그 명칭이 일치할 경우 태그 기반 필터링 우선 적용
  - [**확인 완료**]: 'train' 검색 시 '기차' 관련 태그 트윗이 상단에 노출되는 등 의미론적 연동 확인

- [x] 검색 UI 업데이트
  - [x] 우측 사이드바 "AI 검색" 진입점 신설
  - [x] 검색 결과 페이지 "keyword 검색" 명칭 적용 및 직관적 레이아웃
  - [x] 검색 결과 간격(`gap-4`) 및 시각적 구분 최적화
  - [**확인 완료**]: 사용자가 의사소통하는 방식에 따른 최적화된 검색 경험 제공
...
### 목표
사진 촬영과 음성 녹음만으로 AI가 감성적인 여행기(트윗)를 자동 생성하는 '여행 일지 모드' 구현

### 작업 항목

- [x] 여행 일지 전용 UI (Log Mode Overlay) ✅
  - [x] 상시 노출되는 'AI 여행 일지' 버튼 추가
  - [x] 촬영-녹음-생성으로 이어지는 전용 오버레이/페이지 UI 구현
  - [**확인 완료**]: 플로팅 버튼 클릭 시 카메라 프리뷰 및 스타일 선택기가 포함된 오버레이가 즉시 활성화됨

- [x] 음성 녹음 및 STT 통합 ✅
  - [x] 브라우저 Web Speech API(webkitSpeechRecognition) 연동
  - [x] 실시간 음성 자막 표시 및 텍스트화
  - [**확인 완료**]: 한국어 음성이 실시간으로 텍스트로 변환되어 화면에 표시됨

- [x] 사진(Vision) & 위치(POI) 데이터 통합 ✅
  - [x] Gemini 2.0 Flash-Exp 비전 분석 연동
  - [x] Geolocation & Google Geocoding API 연동 (실제 지명 추출)
  - [x] 사용자 선택형 문체 엔진 (감성/정보/위트/스마트 분석)
  - [**확인 완료**]: 실제 현재 위치(시/구 단위)와 사진 속 사물(예: JBL 스피커)을 AI가 정확히 인지하여 문장에 반영함

- [x] AI 문장 합성 및 발행 프로세스 ✅
  - [x] STT 결과 + 비전 데이터 + 실시간 위치 + 문체를 결합한 최종 트윗 초안 생성
  - [x] Base64 이미지 서버 전송 및 Cloudinary 자동 업로드 연동
  - [x] 유저 컨펌 및 직접 수정 후 즉시 발행 기능
  - [**확인 완료**]: '확인 및 게시' 클릭 시 사진과 함께 풍성한 100자 내외의 여행 로그가 피드에 게시됨
...
- [x] 기본 북마크 기능 ✅
  ...
  - [x] 북마크 버튼 UI
  - [**확인 완료**]:
    - 트윗 카드에 북마크 버튼이 추가되었으며 클릭 시 색상이 변경됨
    - 북마크 클릭 시 즉시 데이터베이스에 반영되며, 페이지 새로고침 후에도 유지됨
    - 북마크 목록 페이지(`/bookmarks`)에서 내가 저장한 트윗들만 모아볼 수 있음
    - 북마크 취소 시 목록에서 실시간으로 사라지거나 새로고침 시 제거됨
...
- [x] 고급 북마크 기능 (확장) ✅
  - [x] 북마크 내 검색 및 필터링
    ...
    - 미디어(사진/영상) 포함 여부 필터링
  - [x] 북마크 폴더/카테고리 (컬렉션)
    - 폴더 생성/수정/삭제 (예: '여행 계획', '맛집', '위시리스트')
    - 북마크 추가 시 폴더 선택 기능
    - 폴더별 북마크 조회 뷰
  - **확인 완료**:
    - 북마크 페이지 상단 검색바에서 검색 시 필터링된 결과가 나옴
    - '미디어만 보기' 필터 적용 시 사진/영상이 있는 트윗만 표시됨
    - 새 폴더를 생성하고 트윗을 해당 폴더에 저장할 수 있음
    - 폴더 탭 이동 시 해당 폴더의 트윗만 표시됨
    - 트윗 카드의 북마크 버튼에서 드롭다운 메뉴로 폴더 선택 가능
    - BookmarkCollection 모델이 데이터베이스에 추가됨
    - API 라우트 `/api/bookmarks/collections` 구현됨

- [x] 알림 시스템 ✅
  - [x] 알림 데이터 모델 (`Notification`)
  - [x] 알림 API (생성, 읽음 처리, 조회)
  - [x] 알림 UI: 알림 페이지 및 배지
  - [x] 실시간 알림 (Polling 방식)
  - **확인 방법**:
    - `Notification` 모델이 `schema.prisma`에 정의되었는지 확인
    - `api.notifications.ts` 라우트가 구현되었는지 확인
    - 좋아요/리트윗/팔로우/답글 시 알림이 생성되는지 확인
    - 알림 페이지(`/notifications`)에서 알림 목록이 표시되는지 확인
    - 사이드바/네비게이션 바에 알림 배지가 표시되는지 확인
    - 알림 읽음 처리가 정상적으로 동작하는지 확인
    - 알림 아이콘에 읽지 않은 알림 개수가 표시되는지 확인
    - 알림 클릭 시 해당 트윗/사용자로 이동하는지 확인
    - 알림 읽음 처리 기능이 동작하는지 확인

- [x] 트렌딩/인기 기능 ✅
  - [x] 인기 여행 태그 표시 (최근 7일간 사용된 태그 기준)
  - [x] 우측 사이드바에 실시간 인기 태그 섹션 추가
  - **확인 완료**:
    - 우측 사이드바에 '인기 여행 태그' 섹션이 표시됨
    - 최근 7일간 가장 많이 사용된 태그가 상위 5개 표시됨
    - 태그 클릭 시 해당 태그 페이지로 이동
    - 게시물 개수가 함께 표시됨
    - 최근 데이터가 없을 경우 전체 인기 태그로 폴백

- [ ] 국내 특화 지도 서비스 (네이버 지도 연동) - 한국 전용 기능
  - 하이브리드 지도 시스템 구현 (해외: Google, 국내: Naver)
  - 사용자 설정에서 '국내 여행 시 네이버 지도 우선 사용' 옵션 제공
  - 네이버 지도 API 연동 및 마커/상세 정보 표시
  - **확인 방법**:
    - 한국 위치 선택 시 네이버 지도가 표시되는지 확인
    - 해외 위치 선택 시 구글 맵이 표시되는지 확인
    - 설정 옵션에 따라 지도 우선순위가 변경되는지 확인

- [x] 여행 경로 시각화 (여행 블로그 특화) ✅
  - [x] 사용자의 여러 트윗 위치를 지도에 표시
  - [x] 여행 경로 연결선 표시 (시간순, 24시간 간격 분리 로직 적용)
  - [x] 프로필 페이지 내 '여행 지도' 탭 구현
  - [x] 마커 클릭 시 게시물 팝업(InfoWindow) 표시
  - **확인 완료**:
    - 프로필 페이지에서 '여행 지도' 탭이 정상적으로 표시됨
    - 지도에 사용자의 여행 위치 마커가 시간순 번호와 함께 표시됨
    - 위치 간 연결선이 올바르게 표시되며, 일정 간격 이상 시 분리됨
    - 마커 클릭 시 사진과 내용 요약이 포함된 인포윈도우가 나타남
    - 모바일 뷰에서도 지도가 올바른 크기로 렌더링됨

- [x] 사용자 프라이버시 및 공개 범위 설정 (Privacy & Visibility) ✅
  - [x] 데이터베이스 스키마 확장 (User.isPrivate, Tweet.visibility, Follow.status)
  - [x] 트윗 작성 시 공개 범위 설정 UI 및 API (전체/팔로워/나만)
  - [x] 사용자 설정에서 비공개 계정 전환 UI
  - [x] 비공개 계정 팔로우 요청 관리 (요청/수락/거절 - 요청 로직 구현됨)
  - [x] 트윗 노출 및 접근 권한 제어 (Feed, Profile, Search)
  - [x] 트윗 수정 시 공개 범위 변경 및 UI 아이콘 표시
  - **확인 방법**:
    - 트윗 작성 및 **수정** 창에서 '전체 공개', '팔로워', '나만 보기'를 선택할 수 있는지 확인
    - 트윗 카드에 공개 범위 아이콘(지구본, 자물쇠 등)이 표시되는지 확인
    - DB에 저장된 Tweet 데이터의 visibility 필드가 올바른지 확인
    - 비공개 계정 설정 시, 팔로우하지 않은 사용자가 프로필/트윗을 볼 수 없는지 확인
    - 팔로우 요청이 '승인 대기(Pending)' 상태로 저장되고 알림이 가는지 확인

- [x] 알림 삭제 기능
  - [x] 알림 개별 삭제 기능 (More 메뉴 연동)
  - [x] 알림 전체 삭제 기능 (/api/notifications DELETE 핸들러 및 UI 연동)
  - [x] 삭제 전 확인 팝업 (전체 삭제 시)
  - **확인 방법**:
    - 알림 목록에서 개별 알림 삭제가 가능한지 확인
    - 알림 전체 삭제 버튼 클릭 시 확인 팝업이 뜨는지 확인
    - 전체 삭제 후 알림 목록이 비워지는지 확인
    - 삭제 후 페이지를 새로고침해도 데이터가 유지되는지 확인

- [x] 위치 기반 피드 (여행 블로그 특화) ✅
  - [x] 특정 국가/도시의 트윗만 보기
  - [x] 홈 피드에 위치 필터 검색 추가
  - **확인 완료**:
    - 홈 피드 상단에 위치 필터 검색창이 추가됨
    - 국가 또는 도시 이름으로 트윗 필터링 가능
    - 필터 적용 시 해당 위치의 트윗만 표시됨
    - 필터 클리어 버튼으로 쉽게 초기화 가능

- [x] 여행 통계 (여행 블로그 특화) ✅
  - [x] 사용자별 여행 통계 (방문 국가 수, 도시 수, 총 여행 일수)
  - [x] 방문 국가 목록 및 Top 5 표시
  - [x] 연도별/월별 여행 기록
  - **확인 완료**:
    - 프로필 페이지에 '여행 통계' 탭이 추가됨
    - 방문 국가, 도시, 여행 일수, 게시물 수 통계 표시
    - 가장 많이 방문한 국가 Top 5 랭킹 표시
    - 연도별 여행 기록 차트 표시
    - 방문한 모든 국가 목록 태그 형태로 표시
    - API 엔드포인트 `/api/travel-stats/:userId` 구현됨
  - **확인 방법**:
...
### [UI First] 1단계: UI 프레임워크 및 더미 데이터 구현

이 단계에서는 백엔드 없이 완벽한 UI/UX를 먼저 완성합니다.

- [x] **1단계: UI 프레임워크 및 더미 데이터 구현** (완료)
  - [x] 메시지 더미 데이터(Mock Data) 정의 (`app/lib/mock-messages.ts`)
  - [x] `MessageDrawer` 컴포넌트 구현 (우측 하단 고정형)
  - [x] `/messages` 및 `/messages/:id` 라우트 UI 구현
  - [x] 기본적인 최소화/최대화 및 리스트/대화 전환 애니메이션

---

### [UI First] 2단계: 상세 UI 컴포넌트 개발 (X 스타일 재현)

- [ ] **대화 목록 컴포넌트**
  - 탭: "전체" / "요청" (X와 동일한 캡슐형 토글 UI)
  - 검색바: 상단 고정 검색바 ("채팅 검색")
  - 리스트 아이템: 프로필 사진, 인증 배지, 이름, 유저네임(@), 마지막 메시지, 시간(상대적 표시)
- [ ] **채팅 인터페이스 컴포넌트**
  - **헤더**: 뒤로가기, 상대 프로필 요약, 설정 아이콘
  - **프로필 정보 카드**: 대화 시작 부분에 표시되는 "프로필 보기" 카드
  - **메시지 버블**: 날짜별 그룹핑 및 본인(오른쪽)/상대(왼쪽) 구분
  - **입력 바**: "+" 버튼, 텍스트 입력, 아이콘 버튼 (복호화된 메시지 텍스트 플레이스홀더)

---

### [UI First] 3단계: 모달 및 설정 UI 개발

- [ ] **새 쪽지 작성 모달**
  - 사용자 실시간 검색 UI 구현
  - "그룹 만들기" 버튼 및 다중 선택 UI
- [ ] **메시지 설정 드로어/페이지**
  - "쪽지 요청 허용" 라디오 버튼 (받지 않음 / 인증된 사용자 / 모든 사람)
  - "스팸 필터링" 토글 스위치
  - "암호화 설정" 등 상세 메뉴 UI
...
### [참고] 세부 API 및 상태 명세 (Technical Specs)

1. **메시지 요청(Request) 로직**:
   - `isAccepted: false`인 대화는 오직 유저의 '요청' 탭에만 표시됩니다.
   - 유저가 메시지를 한 번이라도 보내거나 '승인' 버튼을 누르면 `true`로 전환됩니다.

2. **그룹 채팅 권한**:
   - `DMParticipant.isAdmin` 필드를 통해 멤버 초대 및 그룹명 수정 권한을 관리합니다.
...
model DMConversation {
  ...
  groupName      String?  // 그룹 채팅 이름 (그룹인 경우만)
  lastMessageAt  DateTime @default(now())
  isAccepted     Boolean  @default(false) // 메시지 요청 수락 여부
  createdAt      DateTime @default(now())
  updatedAt      DateTime @updatedAt
...
model DMParticipant {
  ...
  leftAt         DateTime? // 그룹 채팅에서 나간 시간
  isAdmin        Boolean  @default(false) // 그룹 관리자 여부
...
### UI/UX 고려사항 (X 스타일 포인트)

1. **Floating Drawer**: 메인 피드 어디서든 메시지를 빠르게 확인/답장할 수 있는 편의성 제공
2. **Tab Transition**: '전체'와 '요청' 탭 전환 시 부드러운 전환 효과
3. **Empty States**: 메시지가 없을 때의 감각적인 일러스트 또는 안내 문구
4. **Loading Shimmers**: 대화 목록 로딩 시 스켈레톤 UI 적용
...
5. **AI 특화 기능** (Phase 11): AI 여행 일지 모드 (신규)
6. **고급 기능** (Phase 12): 추가 기능
7. **모바일 및 품질** (Phase 13-14): 앱 배포 및 최적화
