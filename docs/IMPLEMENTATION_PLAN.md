...
- [x] 여행 일정 연계 (Travel Plan Integration)
  ...
  - [x] `/api/travel-plans` 엔드포인트 구현 (사용자별 일정 조회)
  ...
  - [ ] 여행 일정 상세 페이지 (`/travel-plans/:id`) 및 그룹화된 트윗 표시
  - **확인 방법**:
    - 사이드바/하단바에 '여행 일정' 메뉴가 추가되었는지 확인
    - `/travel-plans` 페이지에서 사용자의 여행 계획 목록이 보이는지 확인
    - 트윗 작성 시 여행 계획을 선택하고 제출할 수 있는지 확인
    - 작성된 트윗에 여행 계획 뱃지가 표시되는지 확인
    - 트윗 상세 및 피드에서 여행 계획 정보가 올바르게 렌더링되는지 확인
